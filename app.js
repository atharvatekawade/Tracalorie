...
const App=(function(StorageCtrl,ItemCtrl,UICtrl){
    ...
    const loadEventListeners=function() {
        ...
        const clickChart = (e) => {
            ...
            console.log(items);
            let day_calories = [0, 0, 0, 0, 0, 0];

            for(let i=0; i<items.length; i++) {
                let idx = Math.floor(items[i].hour/4);
                day_calories[idx] += items[i].calories;
            }

            let labels = ["00:00 - 04:00", "04:00 - 08:00", "08:00 - 12:00", "12:00 - 16:00", "16:00 - 20:00", "20:00 - 24:00"];
            // labels = [00, 04, 08, 12, 16, 20];
            dataPoints = []

            for(let i=0; i<labels.length; i++) {
                console.log(`${labels[i]}: ${day_calories[i]} calories`);
                dataPoints.push({ y: day_calories[i] });
            }

            UICtrl.renderChart(dataPoints);
